...
# Inicializar estados para evitar saltos de página
...
# --- ESTILOS CSS (ESTRUCTURA SEGURA PARA EL NAVEGADOR) ---
st.markdown("""
    <style>
    ...
    .stButton>button:hover { background: #00f2fe !important; color: #0a192f !important; box-shadow: 0 0 15px #00f2fe; }

    /* Forzar que las imágenes no rompan el layout */
    img { border-radius: 10px; border: 1px solid #00f2fe; }
    </style>
...
st.markdown("<p style='text-align:center; color:#ffd700; font-family:Orbitron; letter-spacing: 2px;'>PRESENTADO POR: LOS ONICHAN 😎</p>", unsafe_allow_html=True)

# BLOQUE DE MISIÓN (Se usa un solo bloque HTML para evitar errores de cierre de etiqueta)
st.markdown(f"""
    <div class="onichan-mission">
        <h3 style="color:#ffd700; margin:0; font-family:'Orbitron'; letter-spacing: 1px;">{st.session_state.mission['task']}</h3>
    </div>
""", unsafe_allow_html=True)
...
with col_cam:
    st.markdown("<h4 style='font-family:Orbitron; color:#00f2fe;'>📡 SCANNER BIOMÉTRICO</h4>", unsafe_allow_html=True)
    
    # IMPORTANTE: El widget está solo, sin estar envuelto en <div> manuales
    foto = st.camera_input("Scanner", label_visibility="collapsed", key="cam_onichan_v3")
    
    if foto:
        if st.button("🔍 INICIAR ANÁLISIS", key="btn_scan"):
            with st.spinner('⚡ ACCEDIENDO A REDES NEURONALES...'):
                ...
                if res_data:
                    ...
                    st.error("Rostro no detectado. Reintente.")

with col_res:
    if st.session_state.res_all:
        # Resultado de Misión con diseño Onichan
        ...
        st.markdown(f"""
            <div style="border: 2px solid {res_color}; padding:15px; border-radius:10px; text-align:center; margin-bottom:20px; background: rgba(0,0,0,0.2);">
                <h2 style="color:{res_color}; margin:0; font-family:Orbitron;">{res_text}</h2>
            </div>
        """, unsafe_allow_html=True)

        st.markdown(f"<h3 style='text-align:center; font-family:Orbitron;'>MODO: {TRADUCCIONES.get(st.session_state.dom_emo, '').upper()}</h3>", unsafe_allow_html=True)
        
        if st.session_state.img_display is not None:
            st.image(st.session_state.img_display, use_container_width=True)

        # SECCIÓN DE IMAGEN DE EMOCIÓN (ONICHAN REF)
        ...
        # Barras de Análisis Espectral
        st.markdown("<p style='margin-bottom:5px; font-weight:bold;'>Análisis Espectral:</p>", unsafe_allow_html=True)
...
for i, nombre in enumerate(nombres):
    with cols_team[i]:
        if INTEGRANTES_FOLDER.exists():
            img_m = next((f for f in os.listdir(INTEGRANTES_FOLDER) if nombre.lower() in f.lower()), None)
            if img_m:
                st.image(str(INTEGRANTES_FOLDER / img_m), use_container_width=True)
        st.markdown(f"<p style='text-align:center; color:#ffd700; font-weight:bold;'>{nombre.upper()}</p>", unsafe_allow_html=True)

# VIDEO FINAL
if INTEGRANTES_FOLDER.exists():
    desp = next((f for f in os.listdir(INTEGRANTES_FOLDER) if "despedida" in f.lower()), None)
    if desp:
        st.markdown("---")
        _, v_col, _ = st.columns([1, 2, 1])
        with v_col:
            st.image(str(INTEGRANTES_FOLDER / desp), use_container_width=True)
            st.markdown("""
                <div style="text-align: center; margin-top: 15px;">
                    <a href="https://www.youtube.com/watch?v=He7dSGhyeHA" target="_blank" 
                       ...
                       border-radius: 5px; font-weight: bold; font-family: 'Orbitron'; display: inline-block; box-shadow: 0 0 10px #00f2fe;">
